...
class ChurnPrep():
    ...
    def __init__(self, train_or_score, effective_date_earliest='2016-01-01', effective_date_latest='2018-09-30', effective_date='2018-09-30',
                       ...
                       period_attribute='CUSTOMER_TENURE', status_flag_churn='Inactive', AUM_reduction_threshold=0.75,
        ...
        self.status_flag_churn = status_flag_churn 
        ...
        if self.train_or_score == 'train':
          ...
          user_inputs_dict = { 'columns_required' : columns_required, 'feature_attributes' : feature_attributes,
              ...
              'status_flag_churn' : status_flag_churn, 'AUM_reduction_threshold' : AUM_reduction_threshold,
    ...
    def define_churn_monthly(self, df):
        
        # defines an churn flag for each month. If the customer was 'inactive' in the month, 
        # or their funds under management dropped by the threshold set or more, 
        # the customer gets a churn flag of 1 for that month
        ...
        # where funds_drop is 1 or the status attribute is 'Inactive' set churn to 1, otherwise 0
        df['churn'] = 0
        df.loc[((df['AUM_reduction_threshold']!=0) & (df['FUNDS_DROP']==1)) | (df[self.status_attribute]==self.status_flag_churn), 
            'churn'] = 1
        # drop columns we don't need
        df.drop('AUM_reduction_threshold', axis=1, inplace=True)
        return df

    def define_churn_overall(self, df):
        # This function flags all records associated with the customer with 1 if the customer experienced churn in any month
        
        # group by customer ID, take max of churn column, gives us a record per customer and tells us if they 
        # experienced churn in any month in the dataset
        df_cust_churn = df.groupby(self.granularity_key)['churn'].max().reset_index()
        df_cust_churn[self.target_attribute] = 0
        df_cust_churn.loc[df_cust_churn['churn']>=1, self.target_attribute] = 1
        df_cust_churn.drop('churn', axis=1, inplace=True)
        # join back to the original dataframe to give us our target
        df = df.merge(df_cust_churn, on=self.granularity_key, how='left')
    ...
    def prep_data(self, df_raw, train_or_score):
        ...
        # call function to define churn flag for each month
        df_raw = self.define_churn_monthly(df_raw)
        # call the function that determines if the customer has churned at any point in their tenure
        df_raw = self.define_churn_overall(df_raw)

        churned_df = df_raw[df_raw[self.target_attribute] == 1].copy()

        print(str(churned_df[self.granularity_key].nunique()) + ' customers churned')

        if train_or_score == 'train':    
            # for each customer who churned, get the month that they first churned and select 1st day of the month
            # subract a random number of months in [0, forecast_horizon] as a cutoff point
            # ensures that churn happened within horecast_horizon of cutoff date

            # filter to include only months where churn=1, get the first occurance per customer
            df_churned_first_occ = churned_df[churned_df['churn']==1].groupby(self.granularity_key)[self.customer_end_date].min().reset_index()
            # note that we pick the 1st of the month as the churn happening 
            # create a random number to be used to select cutoff date before the churn happened and within forecast_horizon
            df_churned_first_occ['RAND'] = np.random.rand(df_churned_first_occ.shape[0])
            df_churned_first_occ['CUTOFF_DATE'] = df_churned_first_occ.apply(lambda x: self.udf_sub_rand_latency(x[self.customer_end_date], x['RAND'], self.forecast_horizon), axis=1)
            # drop columns that we don't need anymore
            df_churned_first_occ.drop([self.customer_end_date, 'RAND'], axis=1, inplace=True)
            ...
            df_churned_first_occ['OBS_MONTH_MIN_OW'] = df_churned_first_occ.apply(lambda x: x['CUTOFF_DATE'] - relativedelta(months=self.observation_window), axis=1)
            ...
            churned_df_b4_cutoff = df_raw.merge(df_churned_first_occ, on=self.granularity_key, how='inner')
            # filter to only include records before the cutoff date
            churned_df_b4_cutoff = churned_df_b4_cutoff[churned_df_b4_cutoff[self.customer_end_date]<churned_df_b4_cutoff['CUTOFF_DATE']].copy()
            ...
            churned_cust_with_enough_history = churned_df_b4_cutoff.groupby(self.granularity_key)[self.customer_end_date].agg([max, min]).reset_index()
            churned_cust_with_enough_history.columns = [self.granularity_key, 'MAX_DATE', 'MIN_DATE']
            churned_cust_with_enough_history['PERIODS_OF_DATA'] = churned_cust_with_enough_history.apply(lambda x: self.udf_n_months(x['MAX_DATE'], x['MIN_DATE']), axis=1)
            # remove anyone without enough observation window months
            churned_cust_with_enough_history = churned_cust_with_enough_history[churned_cust_with_enough_history['PERIODS_OF_DATA']>=self.observation_window]
            # inner join back to churned data to just leave customers who have enough data in observation window
            churned_df_b4_cutoff_filtered = churned_df_b4_cutoff.merge(churned_cust_with_enough_history, on=self.granularity_key, how='inner')
            churned_df_b4_cutoff_filtered.drop(['MAX_DATE', 'MIN_DATE', 'PERIODS_OF_DATA'], axis=1, inplace=True)
            ...
            col_list = [attribute for attribute in self.derive_column_list if attribute in churned_df_b4_cutoff_filtered.columns]
            ...
            churned_df_features = self.derive_features(churned_df_b4_cutoff_filtered, col_list, self.observation_window)
            ...
            churned_df_prepped = churned_df_features.groupby(self.granularity_key).last().reset_index()

            print('Prepped data for ' + str(churned_df_prepped.shape[0]) + ' customers')
            print('Finished prepping for churned customers')

            # start prep for non churned customers
            # very similar to above but we select a random cutoff date 
            non_churned_df = df_raw[df_raw[self.target_attribute] == 0].copy()
            ...
            non_churned_cust_with_enough_history = non_churned_df.groupby(self.granularity_key)[self.customer_end_date].agg([max, min]).reset_index()
            non_churned_cust_with_enough_history.columns = [self.granularity_key, 'MAX_DATE', 'MIN_DATE']
            non_churned_cust_with_enough_history['PERIODS_OF_DATA'] = non_churned_cust_with_enough_history.apply(lambda x: self.udf_n_months(x['MAX_DATE'], x['MIN_DATE']), axis=1)
            # remove anyone that doesn't have observation_window + forecast_horizon months of data
            non_churned_cust_with_enough_history = non_churned_cust_with_enough_history[non_churned_cust_with_enough_history['PERIODS_OF_DATA']>=n_months]
            non_churned_cust_with_enough_history['RAND'] = np.random.rand(non_churned_cust_with_enough_history.shape[0])
            ...
            non_churned_cust_with_enough_history['TEMP_DATE'] = non_churned_cust_with_enough_history.apply(lambda x: x['MAX_DATE'] + relativedelta(months=1-self.forecast_horizon), axis=1)
            non_churned_cust_with_enough_history['CUTOFF_DATE'] = non_churned_cust_with_enough_history.apply(lambda x: self.udf_sub_rand_latency(x['TEMP_DATE'], x['RAND'], x['PERIODS_OF_DATA'] - n_months), axis=1)
            non_churned_cust_with_enough_history.drop('TEMP_DATE', axis=1, inplace=True)
            # join back to non_churned_df and filter to save only records before the cutoff date
            non_churned_df_b4_cutoff_filtered = non_churned_df.merge(non_churned_cust_with_enough_history, on=self.granularity_key, how='inner')
            non_churned_df_b4_cutoff_filtered = non_churned_df_b4_cutoff_filtered[non_churned_df_b4_cutoff_filtered[self.customer_end_date]<non_churned_df_b4_cutoff_filtered['CUTOFF_DATE']].copy()
            # get the start month of the observation window
            non_churned_df_b4_cutoff_filtered['OBS_MONTH_MIN_OW'] = non_churned_df_b4_cutoff_filtered.apply(lambda x: x['CUTOFF_DATE'] - relativedelta(months=self.observation_window), axis=1)
            # drop the columns that we don't need
            non_churned_df_b4_cutoff_filtered.drop(['MAX_DATE', 'MIN_DATE', 'PERIODS_OF_DATA', 'RAND'], axis=1, inplace=True)

            # derive additional features based on list passed in config
            non_churned_df_features = self.derive_features(non_churned_df_b4_cutoff_filtered, col_list, self.observation_window)

            # take the most recent date for each customer
            # keep only the latest month prior to the cutoff for each customer
            # use last here in the event that there is more than 1 record for last date
            non_churned_df_prepped = non_churned_df_features.groupby(self.granularity_key).last().reset_index()

            print('Prepped data for ' + str(non_churned_df_prepped.shape[0]) + ' customers')
            print('Finished prepping for non-churned customers')

            df_prepped = pd.concat([churned_df_prepped, non_churned_df_prepped])
            ...
            df_prepped.drop(['RATIO', 'FUNDS_DROP', 'churn', 'OBS_MONTH_MIN_OW', 'AUM_PREV_MONTH'], axis=1, inplace=True)
            ...
            # save out a list of columns that were created when the training dataset was built
            with open(os.environ.get("DSX_PROJECT_DIR") + '/datasets/training_cols.json', 'w') as f:
                json.dump(list(df_prepped.columns), f)
            ...
            non_churned_cust_with_enough_history = non_churned_cust_with_enough_history[non_churned_cust_with_enough_history['PERIODS_OF_DATA']>=self.observation_window]

            print('We have ' + str(non_churned_cust_with_enough_history.shape[0]) + ' customers to score')
            if non_churned_cust_with_enough_history.shape[0] == 0:
            ...
            missing_data_cust = non_churned_cust_with_enough_history[non_churned_cust_with_enough_history['MAX_DATE'].dt.strftime('%Y%m').astype(int)<int(efd_latest.strftime('%Y%m'))][self.granularity_key]
            ...
            non_churned_cust_with_enough_history['OBS_MONTH_MIN_OW'] = efd_latest + datetime.timedelta(days=1) - relativedelta(months=self.observation_window)
            # join back to non_churned_df 
            # and filter to include only records within the observation window
            non_churned_df_b4_cutoff_filtered = non_churned_df.merge(non_churned_cust_with_enough_history, on=self.granularity_key, how='inner')
            non_churned_df_b4_cutoff_filtered = non_churned_df_b4_cutoff_filtered[non_churned_df_b4_cutoff_filtered[self.customer_end_date]>=non_churned_df_b4_cutoff_filtered['OBS_MONTH_MIN_OW']].copy()
            if non_churned_df_b4_cutoff_filtered.shape[0] == 0:
                print('Error : No customer left for scoring. No customers had data within the observation window')
                sys.exit(1)

            # drop the columns that we don't need
            non_churned_df_b4_cutoff_filtered.drop(['MAX_DATE', 'MIN_DATE', 'PERIODS_OF_DATA'], axis=1, inplace=True)

            col_list = [attribute for attribute in self.derive_column_list if attribute in non_churned_df_b4_cutoff_filtered.columns]
            # call the function to derive summary features
            non_churned_df_features = self.derive_features(non_churned_df_b4_cutoff_filtered, col_list, self.observation_window)

            non_churned_df_features.drop('OBS_MONTH_MIN_OW', axis=1, inplace=True)
            # take the most recent date for each customer
            # keep only the latest month for each customer (the effective date month)
            # use last here in the event that there is more than 1 record for last date
            df_prepped = non_churned_df_features.groupby(self.granularity_key).last().reset_index()
            # add new column for customer tenure
            df_prepped[self.period_attribute] = df_prepped.apply(lambda x: self.udf_n_months(x[self.customer_end_date], x[self.date_customer_joined]), axis=1)
            # drop any column that looks like a date
            for col in df_prepped.columns:
            ...
            with open(os.environ.get("DSX_PROJECT_DIR") + '/datasets/training_cols.json', 'rb') as f:
